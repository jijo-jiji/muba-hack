module unipact::mock_usdc {
    ...
    fun init(witness: MOCK_USDC, ctx: &mut TxContext) {
        let (treasury_cap, metadata) = coin::create_currency(
            ...
            b"USDC",
            b"Mock UniPact USDC",
            b"Testnet stablecoin for UniPact escrow hackathon demo",
            option::none(),
            ctx
        );
...
module unipact::escrow {
    ...
    /// Initializes a new project escrow and deposits funds in one atomic step
    ...
    /// Releases funds atomically from the internal Balance<T> upon passing Gonka AI audit
    public entry fun release_audited_milestone<T>(
        ...
        // 10% Protocol Fee / 90% Student Payout
    ...
    /// Emergency cancellation by client if student ghosts (only before release)
